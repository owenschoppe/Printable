...
var startup = function(){
	...
	document.getElementById('loading').addClassName(localStorage.orientation);
	/*
	try {
	...
	*/
		...
		if(defaultDoc != undefined) {
			docKey = defaultDoc; 
			console.log('localStorage["defaultDoc"] ',defaultDoc);
			gdocs.printDocument(null, processRowsCallback); //In printexport.js
			gdocs.start();
		}
		else {
	...
	*/
	...
	initCheck(localStorage.m, false, 'mForm', 'm');
	initRadio(localStorage.orientation, 'landscape', 'orientation', 'pages', toggleCSS); 
...
var processRowsCallback = function(){
	...
	console.log(localStorage);
	//Init CSS
	toggleCSS((localStorage.orientation=='portrait'? 1 : 0 )); //Important: No callback defined. 
	//1 sec
...
initSelect = function(i, cols, defaultFields) {
	...
	console.log('initSelect', formName, i, localStorage[docKey]);
	//var i = parseInt(formName.substr(5,1)); //Done. TODO: pass in i to boost speed.
	if(!localStorage[docKey]){localStorage[docKey] = '';} //Initialize localStorage for the doc if no record is found.
	var valuesArray = localStorage[docKey].split(","); //Build the array from the string stored in the docKey record.
	...
	if(valuesArray[i]){//If i in the array exists use that. //Should we check that that col still exists in document?
		...
		console.log('No value ', localStorage[docKey]);
		...
		localStorage[docKey] = valuesArray; //Update localStorage to reflect the new array.
